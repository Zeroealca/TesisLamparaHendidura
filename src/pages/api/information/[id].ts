...
interface DataResponse {
    ...
    Síntoma: string[];
    Diagnostico: string[];
    "Sintomas tempranos": string[];
    "Sintomas avanzados": string[];
}
...
const HandlerDiseasesId = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const result = await pool.query(
        `select d.name as Enfermedad, t.name as Tratamiento, s.name as Síntoma, d2.name as Diagnostico, es.name as "Sintomas tempranos", as2.name as "Sintomas avanzados"
        from disaeses d
...
const transform = (data: DataResult[]): DataResponse => {
    ...
    data.forEach((item) => {
        ...
        if (item["Sintomas tempranos"]) {
            if (!result["Sintomas tempranos"].includes(item["Sintomas tempranos"])) {
                result["Sintomas tempranos"].push(item["Sintomas tempranos"]);
            }
        }
        if (item["Sintomas avanzados"]) {
            if (!result["Sintomas avanzados"].includes(item["Sintomas avanzados"])) {
                result["Sintomas avanzados"].push(item["Sintomas avanzados"]);
            }
        }
    });
